...
class CTREvaluator:
    ...
    def score2(self):
        ...
        real_score = np.array(self.hist_real)
        pred_score = np.array(self.hist_pred)

        # Rimuovi eventuali NaN
        mask = ~np.isnan(real_score) & ~np.isnan(pred_score)
        real_score = real_score[mask]
        pred_score = pred_score[mask]

        # Se non ci sono campioni validi, restituisci dei valori molto negativi
        if real_score.size == 0:
            self.hist_pred.clear()
            self.hist_real.clear()
            return (
                0.5,  # ROC AUC di un classificatore casuale
                0.0,  # accuracy peggiore
                0.0,  # f1 peggiore
                float('inf'),  # mse infinito
                float('inf')  # mae infinito
            )

        # Crea le etichette secondo la soglia
        pred_label = np.where(pred_score >= self.threshold, 1.0, 0.0)
        real_label = np.where(real_score >= self.threshold, 1.0, 0.0)

        # Svuota gli storici per la prossima chiamata
        self.hist_pred.clear()
        self.hist_real.clear()

        # Calcola e restituisce le metriche
        return (
            roc_auc_score(real_label, pred_score),
