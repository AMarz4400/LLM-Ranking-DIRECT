...
class Tokenizer:
    def __init__(self, pretrain, max_word=None, keep_head_rate=0.7, drop_stopword=True, drop_uncommen=True):
        ...
        self.stopwords = set(stopwords.words("english")) | set(string.digits + string.punctuation) - {"not"}
        self.segwords = set(string.punctuation)
        self.tokenizer = trfm.AutoTokenizer.from_pretrained(pretrain, cache_dir="./cache", local_files_only=True)
    ...
    def transform(self, tokens):
        # Per BERT, self.tokenizer.cls_token sarà "[CLS]".
        # Per Gemma, sarà probabilmente "<bos>" o un altro token di inizio.
        # In questo modo, il codice funziona per ENTRAMBI i modelli!

        cls_token = self.tokenizer.cls_token if self.tokenizer.cls_token is not None else self.tokenizer.bos_token
        sep_token = self.tokenizer.sep_token if self.tokenizer.sep_token is not None else self.tokenizer.eos_token

        tokens = [cls_token] + self.truncate(tokens) + [sep_token]
