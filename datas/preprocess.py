...
def ensure_item_in_training(root, uid, iid, text, score):
    # Leggi gli item presenti nel training set
    train_items = set()
    with open(root + "/train.json", encoding="utf8") as train_file:
        for row in train_file:
            record = json.loads(row)
            train_items.add(record[iid])

    # Leggi gli item presenti nel validation e test set
    valid_test_items = set()
    with open(root + "/valid.json", encoding="utf8") as valid_file:
        for row in valid_file:
            record = json.loads(row)
            valid_test_items.add(record[iid])
    with open(root + "/test.json", encoding="utf8") as test_file:
        for row in test_file:
            record = json.loads(row)
            valid_test_items.add(record[iid])

    # Identifica gli item presenti solo nel validation o test set
    missing_items = valid_test_items - train_items

    if not missing_items:
        print("All items are present in the training set.")
        return

    # Copia un record contenente l'item mancante dal validation o test set al training set
    for item in missing_items:
        ...
        with open(root + "/valid.json", encoding="utf8") as valid_file:
            for row in valid_file:
                record = json.loads(row)
                if record[iid] == item:
                    with open(root + "/train.json", "a", encoding="utf8") as train_file:
                        train_file.write(json.dumps(record) + "\n")
                    break
            else:
                # Se non trovato nel validation set, cerca nel test set
                with open(root + "/test.json", encoding="utf8") as test_file:
                    for row in test_file:
                        record = json.loads(row)
                        if record[iid] == item:
    ...
    print(f"Added {len(missing_items)} missing items to the training set.")
...
def initialize_dataset(src, format_, pretrain, num_worker=4, valid=0.1, test=0.2, users=None, items=None,
    ...
    print("Begin to initialize dataset %s with random seed %d" % (
    os.path.split(root)[-1].rsplit("_", 1)[0].upper(), seed))
    if users is None:
    ...
    # Assicurati che tutti gli item siano presenti nel training set
    ensure_item_in_training(root, uid, iid, text, score)
